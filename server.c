...
void raspunde(void * arg){
    ...
    while (1) {
        ...
        if ((comanda = strstr(buffer, "anunt")) != NULL && comanda-buffer == 0 && *(comanda+5) == '\n') {
            ...
            char msg[SIZE];
            flag = 0;
            sprintf(msg, "Accident pe Strada %s, reduceti viteza la %dkm/h!", u.nume_strada, u.viteza_max);
            ...
            char msg[SIZE];
            flag = 0;
            sprintf(msg, "Drum in lucru pe Strada %s, reduceti viteza la %dkm/h!", u.nume_strada, u.viteza_max);
            trimiteAnunt(nfds, writefds, tdL.cl, msg);
            strcpy(raspuns, "Am trimis mesajul celorlalti.");
        }

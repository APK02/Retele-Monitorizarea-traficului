...
int main (int argc, char *argv[]) {
    ...
    while(1) {
        ...
        int ready;
        if ((ready = select(sd + 1, &read_fds, NULL, NULL, &tv)) < 0 ) {
            ...
            if (strstr(avertisment, "Am scos anuntul de pe Strada") != NULL) {
                char cmd[SIZE];
                strcpy(cmd, "event2\n");
                TrimiteComanda(cmd, raspuns);
            }
            else if (strstr(avertisment, "Nu exista anunt pe Strada") == NULL) {
                char cmd[SIZE];
                strcpy(cmd, "event\n");
                TrimiteComanda(cmd, raspuns);
            }
